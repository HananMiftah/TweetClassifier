  ...
  const confusionMatrix = () => {
    ...
    annotatedTweets.forEach(tweet => {
      if (tweet.label && tweet.predictedLabel) {
        const actualLabel = tweet.label.toLowerCase().trim();
        const predictedLabel = tweet.predictedLabel.toLowerCase().trim();
        if (matrix[actualLabel] && matrix[actualLabel][predictedLabel] !== undefined) {
